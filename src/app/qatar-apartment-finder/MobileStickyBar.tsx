...
import { DEFAULT_WA_MESSAGE, DEFAULT_WA_MESSAGE_AR, PHONE, buildWaLink, trackCallClick } from "./constants";

const TEXT = {
  en: { call: "Call", whatsapp: "WhatsApp" },
  ar: { call: "اتصال", whatsapp: "واتساب" },
};
...
export default function MobileStickyBar({ lang = "en" }: { lang?: "en" | "ar" }) {
  const t = TEXT[lang];
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 flex items-stretch gap-2 border-t border-gray-100 bg-white/95 p-3 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] backdrop-blur-sm lg:hidden">
      ...
      <TrackedWhatsAppLink
        ...
        className="flex flex-1 items-center justify-center gap-2 rounded-full bg-gold-500 text-sm font-semibold text-ink-950"
      >
