...
const ARIA_LABEL = {
  en: "Chat with a property advisor on WhatsApp",
  ar: "تحدث مع مستشار عقاري عبر واتساب",
};

// Floating WhatsApp bubble, desktop only — this page has no sticky header
// (see page.tsx), so this is the persistent one-click WhatsApp CTA for
// desktop visitors. Hidden below `lg`: MobileStickyBar already docks its
// own WhatsApp button along the bottom edge there, and the matchmaking
// form card (StepProgress + option buttons) sits tall enough in the mobile
// hero that a fixed bubble at this position would otherwise float directly
// on top of — and block taps on — its option buttons.
export default function WhatsAppBubble({ lang = "en" }: { lang?: "en" | "ar" }) {
  return (
    <a
      ...
      className="fixed bottom-6 end-6 z-40 hidden h-14 w-14 items-center justify-center rounded-full text-white shadow-lg shadow-black/25 transition hover:scale-105 lg:flex"
